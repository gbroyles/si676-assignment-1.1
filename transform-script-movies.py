...
movie_metadata_file_path = os.path.join('si-assignment-1.1','movie-item-metadata')

print(movie_metadata_file_path)

file_count = 0

for file in glob.glob('../si676-assignment-1.1/movie-item-metadata/movie-item_metadata-*.json'):
    file_count += 1
    print(file)

print('Located',file_count,'Movie Metadata Files')

list_of_movie_item_metadata_files = list()
for file in glob.glob('../si676-assignment-1.1/movie-item-metadata/movie-item_metadata-*.json'):
    list_of_movie_item_metadata_files.append(file)

print(list_of_movie_item_metadata_files)

print(len(list_of_movie_item_metadata_files))

list_of_movie_item_metadata_files.sort()

for file in list_of_movie_item_metadata_files:
    print(file)

with open(list_of_movie_item_metadata_files[0], 'r', encoding='utf-8') as item:
    print('The File in Question:',list_of_movie_item_metadata_files,'\n')
...
collection_info_csv = 'movie_collection_items_data.csv'

headers = ['source_file', 'item_id', 'title', 'date', 'source_url', 'phys_format', 'dig_format', 'rights']

with open(list_of_movie_item_metadata_files[1], 'r', encoding='utf-8') as data:
...
items_data_file = os.path.join('movie_collection_items_data.csv')
...
collection_info_csv = os.path.join('..','si676-assignment-1.1','collection-project','movie_collection_items_data.csv')
file_count = 0
items_written = 0
error_count = 0
